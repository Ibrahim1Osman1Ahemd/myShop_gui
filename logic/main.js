...
function updateUI() {
  ...
  menuArr.map((item) => {
    let id = item.id
    if (document.getElementById(id) && location.pathname.split("/views/")[1] !== 'menu.html') {
  ...
  if (localStorage.token != null) {
    ...
    if (document.querySelector(".authBtn") != null) {
      document.querySelector(
        ".authBtn"
      ).innerHTML = `<a href="#" onclick="logOut()" class="log-out">تسجيل الخروج </a>`;
    }
  } else {
    document.getElementById("logo").innerHTML = `
            <a href="#">متجري</a>
    ...
    if (document.querySelector(".authBtn") != null) {
      document.querySelector(
        ".authBtn"
      ).innerHTML = `<a href="#" onclick="showPopUp()" id="sign-in">تسجيل الدخول</a>`;
    }
  }
}
updateUI();
...
function displayProducts(
  containerID = "all-prodcutd",
  filter = "",
  onclickOptions = "addProdcutToTheManeu(this.id) , addTheprodcutToTheMenuArr(this.id)",
  deleteBtnClass = "add-to-the-menu-btn-before-clicked",
  configOption = {}
) {
  ...
  axios
    .get(
      `${BaseUrl}/products?limit=${limit}&&page=${page}${filter}`,
      configOption
    )
    .then((res) => {
            ...
            <div  class="products-card" >
              ...
              <img src="${product.image}" alt="صورة${ product.title  }" onclick="getProudctDetial('${window.location.pathname == "/views/prodcuts.html" || url["1"] == "" || url[1].includes("index.html") ? product._id: ""}')">
                    ...
                    <div class="container" > 
                        ...
                        <div class="product-text" onclick="getProudctDetial('${
                          window.location.pathname == "/views/prodcuts.html" ||
                          url["1"] == "" ||
                          url[1].includes("index.html")
                            ? product._id
                            : ""
                        }')">
                        ${
                          ...
                          : `
                            <h3> 
                              ...
                              <span  id="${
                                product._id
                              }" onclick="${onclickOptions}" class="">X</span>
                            </h3>
                          `
                        }
                        ${containerID !== "menu-container" ? "<p>${product.description}</p>": ""}
                    </div>

                    <div class="product-price">
                      <b id="${product._id}price">
                        ${containerID === 'menu-container'?  product.price * menuArr[menuArr.findIndex((item) => item.id === product._id)].quantity : product.price}
                      </b>
                      ${
                        containerID !== "menu-container" ? 
                        `
                          <button id="${
                            product._id
                          }" onclick="${onclickOptions}" class="${deleteBtnClass}"     >${btnContent}</button>
                        
                        ` :
                        `
                        <div>
                          <button onclick="quantityOprations('${product._id}quantity','-', '${product._id}price','${product._id}price/${product.price}')">-</button>
                          <span id="${product._id}quantity">${menuArr[menuArr.findIndex((item) => item.id === product._id)].quantity}</span>
                          <button onclick="quantityOprations('${product._id}quantity','+' ,'${product._id}price','${product._id}price/${product.price}')">+</button>
                        </div>
                        `
                      }
...
function updateProfile(id) {
  ...
  // console.log(id)
  axios
    .put(`${BaseUrl}/users/profile/update`, formData, config)
    .then((res) => {
      ...
      if (id === "profileImg/input") {
        document.getElementById("image-cont").style.backgroundImage = `url(${
...
function deleteProduct(productId) {
  axios
    .delete(`${BaseUrl}/products/${productId}`, config)
    .then((res) => {
      showAlert(res.data.message);
      displayProducts(
        `your-prodcuts-container`,
        `&&userId=${userId}`,
...
function getPrices() {
  return new Promise((resolve, reject) => {
    axios.get(`${BaseUrl}/products/get/prices`).then((res) => {
...
function search(cat, price) {
  ...
  displayProducts(
    "all-prodcutd",
    (filter = `&&categories=${cats}&&prices=${prices}`)
  );
}
// GET ONE PRODUCT
...
function addTheprodcutToTheMenuArr(id) {
  let item = menuArr.find(item => item.id === id);
  console.log(id);
  // if the length of the prdouct in the cart more than 20 we wo'nt to add more product to it
  if(menuArr.length > 20)
  {
    return
  }
  // if the prodcut is not in the cart array we add else we remove it from the cart
  if (!item) {
    menuArr.push({id : id , quantity: 1});
  ...
  {
    // we need to remove an element from the array so we want it to be more than zero
    if (menuArr.length < 1) {
      return;
    }
...
function  removeFromTheCart (item){
  ...
  menuArr.splice(menuArr.indexOf(item), 1);
  localStorage.menuArr = JSON.stringify(menuArr);
  // this to remove the product from cart page after remove it from cart array
  if (document.getElementById("menu-container")) {
    document
      .getElementById(item.id)
      .addEventListener("click", displayTheProdcutsInTheMenuPage());
  }
  ...
  document.getElementById(item.id).innerHTML = "اضف الى السلة";
}
